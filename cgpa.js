  ...
  // Track how many subjects have been added (for numbering)
  ...
  function addSubjectRow() {
    ...
    row.innerHTML =
      ...
      '<input class="subject-credits-input" type="number" min="1" max="10" placeholder="Credits" />' +
  ...
  function calculateCGPA() {
    ...
    for (var i = 0; i < rows.length; i++) {
      ...
      if (creditVal === "" || isNaN(creditVal) || parseFloat(creditVal) <= 0) {
        errorMessages.push("Subject " + rowNum + ": enter a valid credit value.");
        hasError = true;
        continue;
      }
      ...
      // Validate grade
      if (gradeVal === "") {
        errorMessages.push("Subject " + rowNum + ": select a grade.");
        hasError = true;
        continue;
      }
      ...
      totalCreditPoints += credits * gradePoint; // Credit Point = Credits × Grade Point
    }
    ...
    if (totalCredits === 0) {
      showError("Total credits cannot be zero. Please check your inputs.");
      return;
    }
  ...
  function showResult(subjectCount, totalCredits, cgpa) {
    ...
    if (cgpa >= 9) {
      ...
      perfSub.textContent  = "CGPA " + cgpa.toFixed(2) + " — You're in the top tier. Keep it up!";
    } else if (cgpa >= 8) {
      badge.classList.add("perf-excellent");
      perfText.textContent = "Excellent Performance";
      perfSub.textContent  = "CGPA " + cgpa.toFixed(2) + " — Great work. Push a little more for the top!";
    } else if (cgpa >= 7) {
      badge.classList.add("perf-verygood");
      perfText.textContent = "Very Good Performance";
      perfSub.textContent  = "CGPA " + cgpa.toFixed(2) + " — Solid result. A focused effort can take you higher.";
    } else if (cgpa >= 6) {
      badge.classList.add("perf-good");
      perfText.textContent = "Good Performance";
      perfSub.textContent  = "CGPA " + cgpa.toFixed(2) + " — Decent progress. Identify weak areas and improve.";
    } else {
      badge.classList.add("perf-needs");
      perfText.textContent = "Needs Improvement";
      perfSub.textContent  = "CGPA " + cgpa.toFixed(2) + " — Don't give up! Build a consistent study plan.";
    }
  ...
  // Start with 1 subject row on load
  addSubjectRow();
